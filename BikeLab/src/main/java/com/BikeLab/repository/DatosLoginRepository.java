...
public interface DatosLoginRepository extends JpaRepository<DatosLogin, Long> {
    ...
    //Custom query
    @Query(value = "select dl.*,rol.rol ,rol.id from bikelab.datoslogin dl join bikelab.roldatoslogin rdl on dl.id = rdl.usuario_id join bikelab.rol on rdl.rol_id = rol.id", nativeQuery = true)
